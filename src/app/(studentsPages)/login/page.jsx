...
// 1. Isolate the part using the hook into a sub-component
  ...
  return (
    ...
    >
      <svg className="w-5 h-5 flex-shrink-0" viewBox="0 0 24 24">
        <path fill="#EA4335" d="M12 5.04c1.66 0 3.2.57 4.38 1.69l3.27-3.27C17.67 1.62 15.03 1 12 1 7.24 1 3.2 3.73 1.24 7.72l3.84 2.97C6.01 7.29 8.78 5.04 12 5.04z" />
        <path fill="#4285F4" d="M23.49 12.27c0-.81-.07-1.59-.2-2.34H12v4.43h6.46c-.28 1.48-1.12 2.74-2.38 3.58l3.69 2.87c2.16-1.99 3.42-4.92 3.42-8.54z" />
        <path fill="#FBBC05" d="M5.08 14.69c-.24-.72-.38-1.49-.38-2.29s.14-1.57.38-2.29L1.24 7.72C.44 9.34 0 11.12 0 13s.44 3.66 1.24 5.28l3.84-2.59z" />
        <path fill="#34A853" d="M12 23c3.24 0 5.97-1.07 7.96-2.91l-3.69-2.87c-1.02.68-2.33 1.09-4.27 1.09-3.22 0-5.99-2.25-6.96-5.41L1.2 15.49C3.16 19.51 7.2 22 12 23z" />
      </svg>
...
// 2. The Main Page remains the same, just with the Suspense wrapper
  ...
  return (
    <main className="min-h-screen w-full grid grid-cols-1 md:grid-cols-12 bg-slate-50 font-sans selection:bg-blue-200">
      ...
      <section className="hidden md:flex md:col-span-6 lg:col-span-7 bg-blue-900 p-12 flex-col justify-between text-white relative overflow-hidden">
        ...
        <p className="text-xs text-blue-300 relative z-10">&copy; 2026 NEETPrep Engine Inc. All production rights reserved.</p>
      </section>
      ...
      <section className="col-span-1 md:col-span-6 lg:col-span-5 flex flex-col justify-center items-center px-6 sm:px-12 py-12 bg-white">
        <div className="w-full max-w-sm space-y-8">
          <div className="text-center md:text-left space-y-2">
            <h2 className="text-3xl font-bold tracking-tight text-slate-900">Aspirant Login</h2>
            <p className="text-sm text-slate-500">Sign in instantly to access your test dashboard.</p>
          </div>

          {/* This is the magic fix: wrap the button in Suspense */}
          <div className="space-y-4">
            <Suspense fallback={<div className="w-full py-4 text-center">Loading...</div>}>
              <LoginButton />
            </Suspense>
          </div>
